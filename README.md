...
**Search `Midea LAN` in HACS not available now, it will be ready later**
...
2. open HACS, click [Custom repositories], Repository input: `https://github.com/wuwentao/midea_lan`, Category select [Integration]
3. **Restart Home Assistant**.

Option 2:

1. Download `midea_lan.zip` from [Latest Release](https://github.com/wuwentao/midea_lan/releases/latest)
2. copy `midea_lan.zip` to `/custom_components/midea_lan` in Home Assistant.
3. **Restart Home Assistant**.

Once it done, open `[Settings]`, `[Device & services]`, `[Integrations]`, `[Midea Lan]`, do init config and add all your devices.
...
After installation, search and add component `Midea LAN` in Home Assistant integrations page.

Or click [![Configuration](https://my.home-assistant.io/badges/config_flow_start.svg)](https://my.home-assistant.io/redirect/config_flow_start?domain=midea_lan)
...
Configure can be found in `Settings -> Devices & Services -> Midea LAN -> Devices -> Configure`.
...
After configuration, one of few main entity (e.g. climate entity) may be generated . If you want to make the attributes to extra sensor and switch entities, click `Configure` in `Midea LAN` integration card to choose (if your devices supported).
...
we should enable `midea_lan` and `midealocal`
